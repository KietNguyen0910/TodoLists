...
const TaskCard = memo(function TaskCard({
  ...
  return (
    <tr ref={setTaskRef} className={`task-row ${isHighlighted ? 'is-highlighted' : ''}`} onDoubleClick={handleRowDoubleClick} style={{ backgroundColor: useNeutralRowBackground ? '#fff' : hexToRgba(statusConfig.color || '#ffffff', 0.3) }}>
      ...
      <td className="cursor-pointer payroll-cell">{task.payroll ? <span className="software-value" style={{ color: getSoftwareColor(task.payroll) }}>{formatPayroll(task.payroll)}</span> : formatPayroll(task.payroll)}</td>
